	...
	success: function(data) { 
		...
		if (data.result['configuration']!=''){
			$.each(data.result['configuration'], function( key, value ){
				AddTypePlantation($('#table_type_plantation tbody'),key);	
				});
		}
	}
});	
$('#bt_AddTypePlantation').on('click',function(){
	AddTypePlantation($(this).closest('.form-horizontal').find('#table_type_plantation tbody'),'');
});
function AddTypePlantation(el,data){
	el.append($('<tr>')
			.append($('<td>')
        .append($('<div class="input-group">')
          .append($('<span class="input-group-btn">')
            .append($('<a class="btn btn-default btn-sm" data-action="remove">')
